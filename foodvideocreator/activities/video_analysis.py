...
def run_video_analysis(con, *, project_id: str, source_path: str | Path, artifact_root: str | Path, provider: AIProvider, dish_name: str | None = None, run_id: str | None = None, rule_bundle: dict[str,Any] | None = None) -> dict[str, Any]:
    ...
    record_check(con,project_id,"CHECK_VIDEO_ANALYSIS_COMPLETE",artifact_id=art["artifact_id"],artifact_sha256=art["sha256"],measurement={"major_scene_count":len(analysis.get("major_scenes") or []),"facts_visible":len(analysis.get("facts_visible") or []),"facts_unconfirmed":len(analysis.get("facts_unconfirmed") or []),"source_preprocess_keys":sorted(preprocess),"decode":analysis.get("full_decode",{}).get("result")},result="PASS" if complete else "FAIL")
    return {"artifact":art,"analysis":analysis,"dish_identity_check":result,"analysis_complete":"PASS" if complete else "FAIL"}
